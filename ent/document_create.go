...
func (dc *DocumentCreate) SetDate(t time.Time) *DocumentCreate {
...
func (dc *DocumentCreate) AddArticleIDs(ids ...int) *DocumentCreate {
	dc.mutation.AddArticleIDs(ids...)
	return dc
}

// AddArticles adds the "articles" edges to the Article entity.
func (dc *DocumentCreate) AddArticles(a ...*Article) *DocumentCreate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
	return dc.AddArticleIDs(ids...)
}
...
func (dc *DocumentCreate) check() error {
	...
	if _, ok := dc.mutation.Date(); !ok {
		return &ValidationError{Name: "date", err: errors.New(`ent: missing required field "Document.date"`)}
	}
...
func (dc *DocumentCreate) createSpec() (*Document, *sqlgraph.CreateSpec) {
	...
	if nodes := dc.mutation.ArticlesIDs(); len(nodes) > 0 {
