...
func (dq *DocumentQuery) QueryArticles() *ArticleQuery {
	query := &ArticleQuery{config: dq.config}
...
func (dq *DocumentQuery) Clone() *DocumentQuery {
	...
	return &DocumentQuery{
		...
		withArticles: dq.withArticles.Clone(),
...
func (dq *DocumentQuery) WithArticles(opts ...func(*ArticleQuery)) *DocumentQuery {
	query := &ArticleQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withArticles = query
	return dq
}
...
func (dq *DocumentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Document, error) {
	...
	if query := dq.withArticles; query != nil {
		...
		query.Where(predicate.Article(func(s *sql.Selector) {
			s.Where(sql.InValues(document.ArticlesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.document_articles
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "document_articles" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "document_articles" returned %v for node %v`, *fk, n.ID)
			}
