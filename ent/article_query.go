...
func (aq *ArticleQuery) QuerySegments() *SegmentQuery {
	query := &SegmentQuery{config: aq.config}
...
func (aq *ArticleQuery) QueryOwner() *DocumentQuery {
...
func (aq *ArticleQuery) Clone() *ArticleQuery {
	...
	return &ArticleQuery{
		...
		withSegments: aq.withSegments.Clone(),
...
func (aq *ArticleQuery) WithSegments(opts ...func(*SegmentQuery)) *ArticleQuery {
	query := &SegmentQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withSegments = query
	return aq
}

// WithOwner tells the query-builder to eager-load the nodes that are connected to
// the "owner" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *ArticleQuery) WithOwner(opts ...func(*DocumentQuery)) *ArticleQuery {
	query := &DocumentQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *ArticleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Article, error) {
	...
	if query := aq.withSegments; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Article)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Segments = []*Segment{}
		}
		query.withFKs = true
		query.Where(predicate.Segment(func(s *sql.Selector) {
			s.Where(sql.InValues(article.SegmentsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.article_segments
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "article_segments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "article_segments" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Segments = append(node.Edges.Segments, n)
		}
	}
	...
	if query := aq.withOwner; query != nil {
		...
		nodeids := make(map[int][]*Article)
		for i := range nodes {
			if nodes[i].document_articles == nil {
				continue
			}
			fk := *nodes[i].document_articles
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "document_articles" returned %v`, n.ID)
			}
			for i := range nodes {
